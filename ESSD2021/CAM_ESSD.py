...
# out_layer_names = ["layer1", "layer2", "layer3", "layer4"]
# 训练过的模型路径
# resume_path = r"C:\Users\admin\Desktop\fsdownload\set113\cross_validation\test1\epoch_0041_top1_81.000_'checkpoint.pth.tar'"
...
def draw_CAM(model, img_path, save_path, transform=None, visual_heatmap=False, out_layer=None):
    ...
    # features: 指定层的输出, features_flatten: 为继续完成前端传播而设置的变量
    features = img
    start_flatten = False
...
if __name__ == '__main__':
    ...
    DATASET_PATH = r"D:\ESSD2021_CAM"
    category_list = os.listdir(DATASET_PATH)
    for category_name in category_list:
        category_path = os.path.join(DATASET_PATH, category_name)
        for img_name in os.listdir(category_path):
            if img_name.endswith("448_crop.jpg"):
                img_path = os.path.join(category_path, img_name)
                save_path = os.path.join(category_path, img_name.split(".")[0] + '_' + out_layer_name + '.jpg')
                print(save_path)
                draw_CAM(seresnet50, img_path, save_path, transform=transform, visual_heatmap=False, out_layer=out_layer_name)