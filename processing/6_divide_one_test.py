...
def select_instances(instance_dict, species_images_sum):
    min = int(species_images_sum * 0.2)
    max = int(species_images_sum * 0.5)
    instance_list = []
    images_num_list = []
    for i, instance in enumerate(instance_dict):
        if max >= sum(images_num_list) >= min:
            return instance_list
        if sum(images_num_list) < min and sum(images_num_list) + instance_dict[instance] < max:
            instance_list.append(instance)
            images_num_list.append(instance_dict[instance])
    if max >= sum(images_num_list) >= min:
        return instance_list
...
def cross_valid(category_name):
    ...
    if category_name in multi_category_list:
        min_threshold = 1
    instance_list = select_instances(instance_dict, species_images_sum)
    length = len(instance_list)
    print('类别名:{}:'.format(category_name), species_images_sum)
    ...
    for image in annotated_images_folder.glob("*.jpg"):
        ...
        for json_image in json_images_all:
            ...
            if name_in_json == img_name or name_in_json == img_name_copy:
                ...
                if not (img_instance_number in instance_list):
