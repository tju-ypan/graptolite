...
def plot_confusion_matrix(cm, classes,
    ...
    ax.set(
        ...
        title=title)
    ax.set_xlabel('Predicted label', fontsize=30)   # 30
    ax.set_ylabel('Ground truth label', fontsize=30)
    ...
    for i in range(cm.shape[0]):
        for j in range(cm.shape[1]):
            ax.text(j, i, format(cm[i, j], fmt if cm[i, j] > 0 else '.1f'),
                    ha="center", va="center",
                    color="white" if cm[i, j] > thresh else "black", fontsize=12)   # 12
...
def plot_matrix(test_results, test_labels):
    ...
    # 统计准确率较低的类别并找出它们被误识的类别
    # for i in range(confusion_matrix.shape[0]):
    #     line = confusion_matrix[i]
    #     # max_index = np.argmax(line)
    #     list_line = [(i, name_list[i], acc_number) for i, acc_number in enumerate(list(line))]
    #     sorted_line = sorted(list_line, key=lambda x: x[2], reverse=True)
    #
    #     max_index = sorted_line[0][0]
    #     max_name = sorted_line[0][1]
    #     max_acc = sorted_line[0][2]
    #     second_index = sorted_line[1][0]
    #     second_name = sorted_line[1][1]
    #     second_acc = sorted_line[1][2]
    #     # if acc_list[i] < 0.5 and max_index != i:
    #     if acc_list[i] < 0.8:
    #         print("类别{} {}：\n{:.2%}的样本被识别为类别 {}\n{:.2%}的样本被识别为类别{}\n".format(name_list[i], acc_list[i], max_acc / np.sum(line), max_name,
    #                                                                          second_acc / np.sum(line), second_name))
    #         with open('SET95_fallibility.txt', 'a', encoding='utf-8') as f:
    #             f.write("类别{} {}：\n\t{:.2%}的样本被识别为类别 {}\n\t{:.2%}的样本被识别为类别{}\n".format(name_list[i], acc_list[i], max_acc / np.sum(line), max_name,
    #                                                                                second_acc / np.sum(line), second_name))
